...
import { DefaultReport } from './ReportDef';
...
export default async function (
	input: RPSReport.InputFilesArg
): Promise<RPSReport.Report> {
	const wb = xlsx.read(input.billing, { type: 'array' });

	const fileData: TherapyNotesRow[] = xlsx.utils.sheet_to_json(
		wb.Sheets[wb.SheetNames[0]]
	);

	const report = {} as RPSReport.Report;
	const Modes = Object.keys(DefaultReport);
...
function results(appts: Appointment[], mode: keyof RPSReport.Report) {
	const results: any[] = [];
...
function tableData(results: any[], mode: keyof RPSReport.Report) {
